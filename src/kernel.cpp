...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZCuxStake(spend));
    } else {
        ...
        CCuxStake* omnsInput = new CCuxStake();
        omnsInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(omnsInput);
    }
