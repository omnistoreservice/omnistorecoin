...
#define MN_WINNER_MINIMUM_AGE 3600    // Age in seconds. This should be > MASTERNODE_REMOVAL_SECONDS to avoid misconfigured new nodes in the list. 8000
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (addr.GetPort() != 6854) return;
        } else if (addr.GetPort() == 6854)
            return;
        ...
        CTxOut vout = CTxOut((MASTERNODE_COLLATERAL - 0.01) * COIN, obfuScationPool.collateralPubKey);
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 OmniStoreCoin tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 OMNS tx -> 1 confirmation
