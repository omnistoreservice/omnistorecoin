...
// Copyright (c) 2015-2018 The OMNS developers
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("OMNSMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("OMNSMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("OMNSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("omns-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running OMNSMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
