...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zomnss that have been in mempool long
                    //and higher priority to zomnss that are large in value
                    ...
                    // zOMNS spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zOMNS spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zOMNS serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
//begin neg array code
        ...
        if(nHeight>10){
            ...
            if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                ...
                //For the period before v2 activation, zOMNS will be disabled and previous block's checkpoint is all that will be needed
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("OmniStoreCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake())
            pwalletMain->zomnsTracker->RemovePending(pblock->vtx[1].GetHash());
        return error("OmniStoreCoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("OmniStoreCoinMiner started (POS=%s)\n",(fProofOfStake ? "true" : "false"));
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("omnistorecoin-miner");
	...
	MilliSleep(1000);
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zOMNS with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    continue;
                }

                //Sign block with the zOMNS key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("BitcoinMiner(): Signing new block with zOMNS key failed \n");
                    continue;
                }
        ...
        LogPrintf("Running OmniStoreCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
