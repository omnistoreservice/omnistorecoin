...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::OMNS);
    ...
    if (!settings.contains("nAnonymizeOmniStoreCoinAmount"))
        settings.setValue("nAnonymizeOmniStoreCoinAmount", 1000);

    nAnonymizeOmniStoreCoinAmount = settings.value("nAnonymizeOmniStoreCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeOmniStoreCoinAmount"))
        SoftSetArg("-anonymizeomnistorecoinamount", settings.value("nAnonymizeOmniStoreCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in omnistorecoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOmniStoreCoinAmount:
            return QVariant(nAnonymizeOmniStoreCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOmniStoreCoinAmount:
            nAnonymizeOmniStoreCoinAmount = value.toInt();
            settings.setValue("nAnonymizeOmniStoreCoinAmount", nAnonymizeOmniStoreCoinAmount);
            emit anonymizeOmniStoreCoinAmountChanged(nAnonymizeOmniStoreCoinAmount);
            break;
        case CoinControlFeatures:
