...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered omnistorecoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 OMNS.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal omnistorecoin:URI (omnistorecoin:XyZ...?message=example)
            {
...
static void NotifyzOMNSReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzOMNSReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzOMNSReset.connect(boost::bind(NotifyzOMNSReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzOMNSReset.disconnect(boost::bind(NotifyzOMNSReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zomnsTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
