...
#include "ui_zomnscontroldialog.h"
...
std::set<std::string> ZCuxControlDialog::setSelectedMints;
std::set<CMintMeta> ZCuxControlDialog::setMints;

ZCuxControlDialog::ZCuxControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCuxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZCuxControlDialog::~ZCuxControlDialog()
{
    delete ui;
}

void ZCuxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCuxControlDialog::updateList()
{
...
void ZCuxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCuxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCux_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCuxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCuxControlDialog::GetSelectedMints()
{
...
void ZCuxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
