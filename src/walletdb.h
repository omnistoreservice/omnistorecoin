...
public:
    ...
    bool WriteZOMNSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZOMNSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZOMNSSeed_deprecated(uint256& seed);
    bool EraseZOMNSSeed();
    bool EraseZOMNSSeed_deprecated();

    bool WriteZOMNSCount(const uint32_t& nCount);
    bool ReadZOMNSCount(uint32_t& nCount);
