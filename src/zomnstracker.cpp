...
CzOMNSTracker::CzOMNSTracker(std::string strWalletFile)
{
...
CzOMNSTracker::~CzOMNSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzOMNSTracker::Init()
{
...
bool CzOMNSTracker::Archive(CMintMeta& meta)
{
...
bool CzOMNSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzOMNSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzOMNSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzOMNSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzOMNSTracker::GetSerialHashes()
{
...
CAmount CzOMNSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzOMNSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzOMNSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzOMNSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzOMNSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzOMNSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzOMNSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzOMNSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzOMNSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzOMNSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzOMNSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzOMNSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzOMNSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzOMNSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzOMNSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzOMNSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzOMNSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzomns from DB\n", __func__, listDeterministicDB.size());
    }
