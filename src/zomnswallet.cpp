...
CzOMNSWallet::CzOMNSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zomns seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZOMNSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZOMNSSeed_deprecated()) {
                    LogPrintf("%s: Updated zOMNS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zomns seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zomns wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzOMNSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZOMNSCount(nCountLastUsed);
    else if (!walletdb.ReadZOMNSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzOMNSWallet::Lock()
{
    seedMaster = 0;
}

void CzOMNSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzOMNSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZOMNS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzOMNSWallet::LoadMintPoolFromDB()
{
...
void CzOMNSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzOMNSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzOMNSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zomnsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzOMNSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZOMNS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zomnsTracker which also adds to database
    pwalletMain->zomnsTracker->Add(dMint, true);
...
void CzOMNSWallet::SeedToZOMNS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzOMNSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzOMNSWallet::GenerateDeterministicZOMNS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzOMNSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzOMNSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
