...
#ifndef OmniStoreCoin_ZOMNSWALLET_H
#define OmniStoreCoin_ZOMNSWALLET_H
...
public:
    CzOMNSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZOMNS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZOMNS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //OmniStoreCoin_ZOMNSWALLET_H
